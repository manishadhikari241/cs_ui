<template>
  <div :class="`component designtile designtile-${isExclusive}`" @click="navigateOrQuickDisplay" v-if="!isExclusive && isGoods && goods">
    <div v-if="!isExclusive && !isGoods" class="img-exclusive-false">
     ...
     <div v-else-if="!isExclusive && isGoods " class="img-exclusive-false">
       <!-- {{goods ? goods.image :'no'}} -->
       <!-- {{goods.image}} -->
            <div class="good-container":style="`background-image: url(/api/v1/image/thumbnail/design/${design.code}/tiny)`">
                                <img class="img-fluid" :src="`/cloudfront/uploads/good/${goods.image}`"/>
                            </div>
    </div>
    ...
    <div v-else class="img-exclusive-true">
      ...
      <template v-else>
        ...
        <div class="icon">
          <i v-if="design.request_status == 0" class="fas fa-hourglass-end"></i>
          <i v-if="design.request_status == 8" class="fas fa-times-circle"></i>
        </div>
      </template>
    </div>
    ...
    <div class="overlay" @click="setMask(goods)" >
      ...
      <template v-else>
        <div class="actions-wrapper-exclusive">
          <div class="actions md-hide">
            ...
            <button class="btn-info" @click.stop="showInfo"><i class="fas fa-info"></i></button>
          </div>
        </div>
        <button v-if="design.request_status == 8" class="btn-delete" @click.stop="deleteDesign">
          <b-icon-x></b-icon-x>
        </button>
      </template>
    </div>
    <ListsPopover :design="design" :target="`design-list-${design.id}`" v-if="!isExclusive" />
  </div>

  <div :class="`component designtile designtile-${isExclusive}`" @click="navigateOrQuickDisplay" v-else-if="!isExclusive && !isGoods">
    <div v-if="!isExclusive && !isGoods" class="img-exclusive-false">
      <img v-if="!isExclusive" :src="`/api/v1/image/thumbnail/design/${design.code}/tiny`"
           :srcset="`/api/v1/image/thumbnail/design/${design.code}/tiny 400w, /api/v1/image/thumbnail/design/${design.code} 800w`"
           sizes="(max-width: 479px) 48vw, 25vw">
  ...
  <div :class="`component designtile designtile-${isExclusive}`" @click="navigateOrQuickDisplay" v-else-if="isExclusive">
    <div v-if="!isExclusive && !isGoods" class="img-exclusive-false">
      <img v-if="!isExclusive" :src="`/api/v1/image/thumbnail/design/${design.code}/tiny`"
           :srcset="`/api/v1/image/thumbnail/design/${design.code}/tiny 400w, /api/v1/image/thumbnail/design/${design.code} 800w`"
           sizes="(max-width: 479px) 48vw, 25vw">
...
export default {
  props: ["design", "topIndex", "isExclusive","goods","isGoods"],
  components: {
    BIconCheck,
    BIconDownload,
    BIconInfo,
