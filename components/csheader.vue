<template>
  <div class="component csheader">
    <nuxt-link
     to="/cms/pages"
     id="cmsIndicator"
     class="d-block text-white text-center font-weight-bold"
     v-if="isCmsUser()">CMS
    </nuxt-link>
    ...
    <b-navbar style="justify-content: space-between;">
      ...
      <b-navbar-nav class="right-navigation">
        <li class="nav-item lg-hide change-lang" v-if="!$auth.loggedIn && $i18n.locale == 'en'">
          <a :href="switchLocalePath('ch')" class="nav-link">简体</a>
        </li>
        <li class="nav-item lg-hide change-lang" v-if="!$auth.loggedIn && $i18n.locale == 'ch'">
          <a :href="switchLocalePath('en')" class="nav-link">English</a>
        </li>

        <b-nav-item href="#" class="lg-hide sign-in" v-if="!$auth.loggedIn" @click="
            setAuthModalMode(1);
            $bvModal.show('modal-auth');
          ">{{ $t("log_in") }}</b-nav-item>
        <b-nav-item href="#" class="lg-hide join-now " v-if="!$auth.loggedIn" @click="
            setAuthModalMode(2);
            $bvModal.show('modal-auth');
          ">{{ $t("join_now") }}</b-nav-item>

        <li class="nav-item lg-hide" v-if="$auth.loggedIn">
          <nuxt-link :to="localePath('/lists')" class="nav-link">{{
            $t("list")
          }}</nuxt-link>
        </li>
        <li class="nav-item lg-hide" v-if="$auth.loggedIn">
        ...
        <b-nav-item-dropdown class="etc" right no-caret>
          ...
          <b-dropdown-item href="#" class="lg-show sign-in" v-if="!$auth.loggedIn" @click="
              setAuthModalMode(1);
              $bvModal.show('modal-auth');
            ">{{ $t("log_in") }}</b-dropdown-item>
          <b-dropdown-item href="#" class="lg-show join-now" v-if="!$auth.loggedIn" @click="
              setAuthModalMode(2);
              $bvModal.show('modal-auth');
            ">{{ $t("join_now") }}</b-dropdown-item>
          <li class="nav-item lg-show" v-if="$auth.loggedIn">
            <a :href="localePath('/user/quota')" class="nav-link">{{ $auth.user.first_name }}&nbsp;&nbsp;<b-icon-person>
              </b-icon-person></a>
          </li>

          <li class="nav-item">
            <a :href="localePath('/about')" class="nav-link">{{
              $t("about_us")
            }}</a>
          </li>
          <li class="nav-item">
          ...
          <li class="nav-item">
            <a :href="localePath('/legal/terms-of-use')" class="nav-link">{{
              $t("legal")
            }}</a>
          </li>
          <li class="nav-item">
          ...
          <li class="nav-item change-lang" v-if="$i18n.locale == 'en' && $auth.loggedIn">
            <a :href="switchLocalePath('ch')" class="nav-link">简体</a>
          </li>
          <li class="nav-item change-lang" v-if="$i18n.locale == 'ch' && $auth.loggedIn">
            <a :href="switchLocalePath('en')" class="nav-link">English</a>
          </li>
          <b-dropdown-item href="#" class="log-out" v-if="$auth.loggedIn" @click="logout()">
            {{ $t("log_out") }}&nbsp;&nbsp;<b-icon-power></b-icon-power>
          </b-dropdown-item>
        </b-nav-item-dropdown>
      </b-navbar-nav>
    </b-navbar>
  ...
  export default {
    ...
    methods: {
      isCmsUser() {
        return (
          this.$auth.loggedIn &&
          (this.$auth.user.role_id == 1 || this.$auth.user.role_id == 2)
        );
      },
    ...
    mounted() {
      if (this.$auth.loggedIn) {
        ...
        this.$axios
          .$patch(`/users/${this.$auth.user.id}/lang-pref`, {
            lang: lang
          })
          .then(response => {
