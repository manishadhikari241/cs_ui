<template>
  <div class="cmscomponent list">
    ...
    <b-table id="cms-colors-list" :items="items" :fields="fields" :busy="loading" :select-mode="'single'" @row-clicked="select" sticky-header small selectable striped></b-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    load(page) {
      this.loading = true;
      this.$axios.$get(`/cms/colors?page=${page}${this.searchTerm.value ? '&term='+this.searchTerm.value : ''}`)
        .then((response) => {
    ...
    select(item, index, event) {
      this.$router.push(`/cms/colors?id=${item.id}`);
    },
