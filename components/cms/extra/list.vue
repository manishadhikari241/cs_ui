<template>
  <div class="cmscomponent list">
    ...
    <b-table id="cms-posts-list" :items="items" :fields="fields" :busy="loading" :select-mode="'single'" @row-clicked="select" sticky-header small selectable striped>
      <template v-slot:cell(cover)="data">
        <img style="height: 30px;" :src="`${imgBaseURL}/storage/${data.item.cover}`">
      </template>
    </b-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    load(page) {
      this.loading = true;
      this.$axios.$get(`/cms/posts?page=${page}${this.searchTerm.value ? '&term='+this.searchTerm.value : ''}`)
        .then((response) => {
    ...
    select(item, index, event) {
      this.$router.push(`/cms/extra?id=${item.id}`);
    },
