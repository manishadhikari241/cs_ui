<template>
  <div class="cmscomponent item category">
      ...
      <form @submit.prevent="update">
        <div class="item-content">
          <div class="item-row">
            ...
            <div class="value">
              <b-input type="text" name="nameEN" :value="item.translations[0] && item.translations[0].tags ? item.translations[0].tags : ''" required></b-input>
            </div>
          </div>
          <div class="item-row">
            <div class="prop">Name (CH)</div>
            <div class="value">
              <b-input type="text" name="nameCH" :value="item.translations[1] && item.translations[1].tags ? item.translations[1].tags : ''" required></b-input>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    load() {
      this.$axios.$get(`/cms/categories/${this.itemId}`)
        .then((response) => {
          this.item = response;
        });
    },
    ...
    update(e) {
      this.loading = true;
      this.$axios.$patch(`/cms/categories/${this.itemId}`, {
      ...
      this.$bvModal.msgBoxConfirm('Are you sure?', {centered: true, bodyClass: 'confirm-box-body', footerClass: 'confirm-box-footer'})
          .then(value => {
            if (value) {
              ...
              this.$axios.$delete(`/cms/categories/${itemId}`)
                .then((response) => {
