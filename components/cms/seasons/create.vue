<template>
  <div class="cmscomponent item season create">
    <div class="title">
      <span><i class="fas fa-plus"></i>&nbsp;&nbsp;ID: Add new season</span>
...
export default {
  ...
  methods: {
    ...
    create() {
      this.loading = true;
      this.$axios.$post('/cms/seasons', this.item)
        .then((response) => {
          this.loading = false;
          this.$toast.success('New season added successfully');
          this.cancel();
        }).catch((error) => {
          this.loading = false;
