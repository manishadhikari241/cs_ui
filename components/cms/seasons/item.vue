<template>
  <div class="cmscomponent item season">
      ...
      <form @submit.prevent="update">
        <div class="item-content">
          <div class="item-row">
            ...
            <div class="value">
              <b-input type="text" name="nameEN" :value="item.translations[0] && item.translations[0].name ? item.translations[0].name : ''" required></b-input>
            </div>
          </div>
          <div class="item-row">
            <div class="prop">Name (CH)</div>
            <div class="value">
              <b-input type="text" name="nameCH" :value="item.translations[1] && item.translations[1].name ? item.translations[1].name : ''" required></b-input>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    load() {
      this.$axios.$get(`/cms/seasons/${this.itemId}`)
        .then((response) => {
          this.item = response;
        });
    },
    ...
    update(e) {
      this.loading = true;
      this.$axios.$patch(`/cms/seasons/${this.itemId}`, {
      ...
      this.$bvModal.msgBoxConfirm('Are you sure?', {centered: true, bodyClass: 'confirm-box-body', footerClass: 'confirm-box-footer'})
          .then(value => {
            if (value) {
              ...
              this.$axios.$delete(`/cms/seasons/${itemId}`)
                .then((response) => {
                  this.$toast.success('Season deleted successfully');
                  this.cancel();
                }).catch((error) => {
                  this.$toast.error(error.response.data.error.message);
                });
