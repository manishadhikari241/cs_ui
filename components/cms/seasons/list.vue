<template>
  <div class="cmscomponent list">
    <div class="actions-container">
      ...
      <div>
        <b-pagination
          ...
          :total-rows="total"
          :per-page="perPage"
          aria-controls="cms-seasons-list"
          @change="load"
        ></b-pagination>
      </div>
    </div>
    ...
    <b-table id="cms-seasons-list" :items="items" :fields="fields" :busy="loading" :select-mode="'single'" @row-clicked="select" sticky-header small selectable striped></b-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    load(page) {
      this.loading = true;
      this.$axios.$get(`/cms/seasons?page=${page}${this.searchTerm.value ? '&term='+this.searchTerm.value : ''}`)
        .then((response) => {
    ...
    select(item, index, event) {
      this.$router.push(`/cms/seasons?id=${item.id}`);
    },
