<template>
  <div class="cmscomponent list">
    <div class="actions-container">
      ...
      <div>
        <b-pagination
          ...
          :total-rows="total"
          :per-page="perPage"
          aria-controls="cms-exclusive-list"
          @change="load"
        ></b-pagination>
      </div>
    </div>
    ...
    <b-table id="cms-exclusive-list" :items="items" :fields="fields" :busy="loading" :select-mode="'single'" @row-clicked="select" sticky-header small selectable striped>
...
export default {
  ...
  methods: {
    load(page) {
      this.loading = true;
      this.$axios.$get(`/cms/requests/exclusives?page=${page}${this.searchTerm.value ? '&term='+this.searchTerm.value : ''}`)
        .then((response) => {
    ...
    select(item, index, event) {
      this.$router.push(`/cms/exclusive?id=${item.id}`);
    },
