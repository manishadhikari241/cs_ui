<template>
  <div class="cmscomponent item exclusive">
    ...
    <div v-if="item">
      ...
      <div class="general">
        ...
        <div class="info-row" v-if="item.status != 8">
          <div class="prop">Design</div>
          <div class="value">
            <div v-if="item.status == 0">
              <b-input v-model="approve.id" style="width: 300px;" placeholder="Link Design ID"></b-input>
              <br>
              <div style="text-align: right;">
...
export default {
  ...
  methods: {
    load() {
      this.$axios.$get(`/cms/requests/exclusives/${this.itemId}`)
        .then((response) => {
          this.item = response;
        });
    },
    ...
    rejectRequest() {
      this.loading = true;
      this.$axios.$post(`/cms/requests/exclusives/${this.itemId}/reject`, { message: this.reject.message })
        .then((response) => {
          this.loading = false;
    ...
    approveRequest() {
      this.loading = true;
      this.$axios.$post(`/cms/requests/exclusives/${this.itemId}/approve`, { designID: this.approve.id })
        .then((response) => {
          this.loading = false;
