      ...
      <form @submit.prevent="update">
        <div class="item-content">
          <div class="item-row">
            ...
            <div class="value">
              <b-input type="text" name="iso2" :value="item.iso2" maxlength="2" required></b-input>
            </div>
          </div>
          <div class="item-row">
...
export default {
  ...
  methods: {
    load() {
      this.$axios.$get(`/cms/countries/${this.itemId}`)
        .then((response) => {
          this.item = response;
        });
    },
    ...
    update(e) {
      this.loading = true;
      this.$axios.$patch(`/cms/countries/${this.itemId}`, {
        iso2: e.target.elements.iso2.value,
      ...
      this.$bvModal.msgBoxConfirm('Are you sure?', {centered: true, bodyClass: 'confirm-box-body', footerClass: 'confirm-box-footer'})
          .then(value => {
            if (value) {
              ...
              this.$axios.$delete(`/cms/countries/${itemId}`)
                .then((response) => {
