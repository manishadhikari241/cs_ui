        ...
        <div class="d-flex flex-wrap justify-content-center">
          <div class="my-3 mx-3">
            ...
            >
              <div class="request border-0">
                <div class="icon">
                  <img
                    src="~/assets/icons/add_symbol.jpeg"
          ...
          <div class="my-3 mx-3" v-for="request in requests" :key="request.id">
            ...
            >
              ...
              <div class="icon" v-if="request.status == 8">
                <i class="fas fa-times-circle"></i>
              </div>
              <div v-if="request.status == 2">
                <nuxt-link
                  :to="localePath(`/designs/${request.designs[0].code}`)"
                >
                  <img
                    v-if="request.designs.length"
                    :src="
                      `${serverURL}/api/v1/image/thumbnail/design/${request.designs[0].code}/tiny`
                    "
                  />
                </nuxt-link>
              </div>
              <div v-if="request.status == 2" class="name-overlay">
                {{ request.name }}
              </div>
              <div v-if="request.status == 8" class="name-overlay">
                {{ request.name }}
              </div>
              <div v-if="request.status == 0" class="name-overlay">
                {{ request.name }}
              </div>

              <b-button class="btn-info" @click="showInfo(request)"
                ><i class="fas fa-info"></i
              ></b-button>
              <b-button
                v-if="request.status == 8"
                class="btn-delete"
                @click="deleteDesign(request.id)"
                ><i class="fas fa-times"></i
              ></b-button>
            </div>
          </div>
          <!-- dont remove these div -->
          <div class="request empty  mx-3"></div>
          <div class="request empty  mx-3"></div>
          <div class="request empty  mx-3"></div>
          <div class="request empty  mx-3"></div>
          <!-- dont remove these div -->
