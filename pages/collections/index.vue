<template>
  <div class="page collections">
    ...
    <div class="search-details">
      <b-dropdown class="season-dropdown" v-if="tmpSeason" :text="tmpSeason[$i18n.locale]" variant="outline">
        <b-dropdown-item href="#" v-for="season in seasons" :key="season.id" @click="tmpSeasonTerm = season.id; search();">{{ season[$i18n.locale] }}</b-dropdown-item>
      </b-dropdown>
      <span v-else>
      ...
      <a class="reset" v-if="term || seasonTerm" @click="reset">{{ $t('reset') }}</a>
    </div>
    <br>
    <div class="collections-list">
      <client-only>
        <Collection v-for="(collection, $index) in data" :key="collection.id" :data="collection" :slideShowLeft="$index%2===0" />
...
export default {
  ...
  methods: {
    loadMore() {
      this.$axios.$get(`/collections?scope[]=user&scope[]=designs&scope[]=moodboards&scope[]=categories&scope[]=goods&scope[]=season&take=${this.itemsPerPage}&sort=-published_at${this.term ? '&keyword='+this.term : ''}${this.seasonTerm ? '&seasonId='+this.seasonTerm : ''}&page=${this.page+1}`)
        .then((response) => {
    ...
    reset() {
      this.$store.dispatch('search/search', {searchDesigns: false, term: '', seasonTerm: '', localePathFunction: this.localePath});
    },

    search() {
      this.$store.dispatch('search/search', {searchDesigns: false, term: this.term, seasonTerm: this.tmpSeasonTerm, localePathFunction: this.localePath});
    }
  },
  watch: {
