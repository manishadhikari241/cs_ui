<template>
  ...
  <div class="page extra">
    ...
    <client-only>
      <nuxt-link v-if="first" class="post cover" :to="localePath(`/extra/${first.id}`)">
        <div class="image-container" :style="`background-image: url('${imgBaseURL}/storage/${first.cover}')`"></div>
        <div class="title">{{ first.translations[($i18n.locale == 'en' ? 0 : 1)].title }}</div>
      </nuxt-link>
        ...
        <b-row>
          <b-col md="4" v-for="post in data" :key="post.id">
            <nuxt-link class="post" :to="localePath(`/extra/${post.id}`)">
              <div class="image-container" :style="`background-image: url('${imgBaseURL}/storage/${post.cover}')`"></div>
              <div class="title">{{ post.translations[($i18n.locale == 'en' ? 0 : 1)].title }}</div>
            </nuxt-link>
          </b-col>
        </b-row>
      </div>
...
export default {
  ...
  asyncData (context) {
    return context.$axios.get('/pages/extra')
      .then((res) => {
