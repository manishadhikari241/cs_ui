<template>
  <div class="page list">
    <div style="text-align: center" v-if="!list"><b-spinner type="grow" label="Loading..."></b-spinner></div>
    <div v-if="list">
      <b-container class="list-header">
        <div v-show="!editTitleMode">
          <h3>
            <span>{{ list.name }}</span>
            <b-button size="sm" variant="outline-secondary" class="btn-edit-list-title ignorePrint" @click="editTitleMode = true; $nextTick(() => { $refs.updateListTileInput.focus(); });">
        ...
        <div v-show="editTitleMode">
          <form @submit.prevent="updateListTitle">
            <b-input-group>
              <b-form-input ref="updateListTileInput" name="name" :value="list.name" v-on:blur="editListNameBlur" required></b-form-input>
              <b-input-group-append>
                <b-button class="updateListTileButton" variant="dark" type="submit" :disabled="loading">{{ $t('update') }}</b-button>
              </b-input-group-append>
            </b-input-group>
          </form>
        </div>
        ...
        <div class="share-box">
          ...
          <div class="share-body" v-show="shareExpanded">
            <div class="list-url-box">
              ...
              <p>{{ getListShareURL() }}</p>
              <div class="actions">
                <a :href="getListShareURL()" target="_blank" v-b-tooltip.hover :title="$t('preview_share_link')"><b-icon-eye-fill></b-icon-eye-fill></a>
                <b-button v-b-tooltip.hover :title="clipboardTooltip" @click="copyListShareURL"><b-icon-clipboard></b-icon-clipboard></b-button>
              </div>
            </div>
                ...
                <template v-for="(n, index) in shareEmailCount">
                  <div :key="`recipient_email_${n}`" style="display: flex;">
                    <b-input
                      ...
                      :required="index == 0"
                    />
                    <b-button tabindex="-1" v-if="index == shareEmailCount-1" class="addShareRecipient" @click="addRecipientInput">
