            ...
            <div class="payments-container" v-show="!invoice">
              ...
              <b-table :items="payments" :fields="fields" :responsive="'md'" striped>
                ...
                <template v-slot:cell(amount)="data">
                  ...
                  <b-badge variant="primary" v-if="data.item.channel == 'coupon'">{{ $t('coupon') }}</b-badge>
                </template>
                ...
                <template v-slot:cell(actions)="data">
                  <b-button @click="showInvoice(data.item)" style="text-transform: capitalize">{{ $t('view') }}</b-button>
                </template>
              </b-table>
            </div>
            ...
            <div class="invoice-container" v-if="invoice">
              <div class="title">
               ...
               <div class="row buttons d-flex">
                    <button class="btn-back ignorePrint" @click="showInvoice(null)"><i class="fa fa-arrow-left" aria-hidden="true"></i> {{ $t('back') }}</button>
                <button size="sm" @click="print" class="btn-print ignorePrint">{{ $t('print_invoice') }}
              ...
              <div class="invoice-details">
                ...
                <div class="value">
                  <div v-if="invoice.package == 'standard' || invoice.package == 'extended'">
                    <p><span class="capitalize">{{ $t(invoice.package) }}</span> {{ invoice.package == 'standard' || invoice.package == 'extended' ? 'License' : 'Requests' }} - ${{ invoice.amount }} for {{ invoice.quantity }} {{ invoice.package == 'standard' || invoice.package == 'extended' ? 'designs' : 'requests' }}</p>
                    <p v-if="invoice.package == 'standard'">{{ $t('jpg_only') }}</p>
                    <p v-if="invoice.package == 'extended'">{{ $t('jpg_pdf_ai_eps') }}</p>
                    <p v-if="invoice.package == 'standard'">{{ $t('for_print_advertising_design') }}</p>
                    <p v-if="invoice.package == 'extended'">{{ $t('for_resale_items_or_reproduction') }}</p>
                  </div>
                  <div v-else-if="invoice.package == 'simulator'">
                    <p>PRODUCT SIMULATOR - ${{ invoice.amount }} for {{ invoice.quantity }} products</p>
                    <p>Add your own products into the simulator</p>
                  </div>
                  <div v-else>
                    <p>Exclusive Designs - ${{ invoice.amount }} for {{ invoice.quantity }} designs</p>
                    <p>JPG, PDF, AI, EPS</p>
                ...
                <div class="value">{{ $t('one_year') }} : {{ $moment(invoice.created_at).format('DD/MM/YYYY') }} - {{ $moment(invoice.created_at).add('years', 1).format('DD/MM/YYYY') }}</div>
